...
**Tela completa disponível a partir do Milestone 4** (analytics de gasto da persona leiga) — é onde `group_by_payee`, `extremes`, `anomaly`, `credit::utilization`, `open_installments`, `scheduled_payments` e o drill-down existem de fato; não desenhar esta tela completa antes disso. Exceção: `period_series` (evolução mês a mês) já existe desde o Milestone 3 (Jornada 3) — pode ser prototipado isoladamente antes do resto da tela, mesmo que a composição densa abaixo só feche no Milestone 4.
