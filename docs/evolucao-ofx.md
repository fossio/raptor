...
**Itens:** aplicar as rodadas de auditoria pendentes (achados #48–#85); corpus por dialeto dos principais emissores brasileiros, semeado pelas dores públicas do §3 (sinal sem `-`, arquivos mistos); paridade sgml/xml (#55); separação estrutural do Discovery. (Item removido em 2026-07-27: "parcelamento como campo estruturado" — achado #76 revertido, decisão do autor de trabalhar estritamente com o spec OFX, sem campo `installment` derivado.)
...
1. **Projeção de fluxo e de fatura.** Recorrências do H1 + parcelas em andamento → forecast da próxima fatura e do saldo em N dias, **como intervalo com premissas na proveniência**, nunca ponto seco. Racional: é a resposta local-first ao forecast do Firefly III; parcelamento dá ao raptor uma vantagem estrutural no caso brasileiro — parcelas futuras são o componente *determinístico* do forecast de fatura, e nenhuma ferramenta do §3 explora isso bem. (Pré-requisito não trivial: o achado #76 — campo `installment` estruturado — foi revertido em 2026-07-27, spec-only por decisão do autor; este item do H2 reabriria essa pergunta se algum dia for promovido, não herda a decisão atual automaticamente.)
...
2. **CSV/planilha como formato de entrada.** **Decisão do autor (2026-07-27): não decidir agora** — ideia de horizonte, não compromisso; foco 100% OFX vale também para CSV, não só para Open Finance. Racional original, preservado como referência: realidade brasileira — parte dos emissores não fornece OFX (limitação reconhecida pelos próprios apps do §3, que aceitam planilha como fallback). Entraria como terceiro parser independente (`csv::to_document` com perfil de mapeamento por dialeto) — e seria o gatilho registrado na ADR-02/§11 para reabrir a pergunta do AST neutro com 3 formatos.
